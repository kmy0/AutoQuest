...
local function parse_quest_data(quest_data)
	...
	if research.request then
		...
		research.level =
			methods.get_limit_lvl:call(mysterylabo, research.request:get_field("_QuestCondition"))
	end
	...
	for no, quest in pairs(quest_data) do
		...
		if
			...
			and quest.type ~= dump.quest_types["ARENA"]
		then
		...
		elseif
			quest.type ~= dump.quest_types["TOUR"]
			and quest.type ~= dump.quest_types["COLLECTS"]
		then
		...
		if quest.category == "Random Mystery" or quest.category == "Special Random Mystery" then
			if
				methods.random_mystery_quest_auth:call(singletons.questman, quest.data, false) ~= 0
			then
		...
		quest.is_completed =
			methods.is_quest_clear:call(singletons.progquestman, functions.sanitize_quest_no(no))

		if
			quest.category == "Event"
			and quest.data:get_field("_EnemyLv") == dump.ranks_ids["High"]
			and not highrank_unlock
		then
			quest.is_unlocked = false
		elseif quest.category == "Event" then
			quest.is_unlocked = true
		else
			quest.is_unlocked = methods.is_quest_unlocked:call(
...
function dump.random_mystery()
	...
	for _, quest in
		pairs(functions.to_array(singletons.questman:get_field("_RandomMysteryQuestData")))
	do
...
function dump.quest_data()
	local no = nil
	local quest_data = {}
	local quest_dict = functions.to_array(
		singletons.questman:get_field("_QuestDataDictionary"):get_field("_entries")
	)
	dump.quest_data_list = {}
	...
	for _, quest in
		pairs(functions.to_array(singletons.questman:get_field("_DlQuestData"):get_field("_Param")))
	do
...
function dump.load()
	...
	if not dump.non_custom_quest_ids then
		dump.non_custom_quest_ids = {}
	end
	dump.anomaly_investigations_main_monsters =
		json.load_file(dump.anomaly_investigations_main_monsters_file_name)
