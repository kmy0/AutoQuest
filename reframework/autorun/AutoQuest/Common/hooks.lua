...
function common_hooks.hook()
	sdk.hook(methods.online_warn, function(args)
		return sdk.PreHookResult.SKIP_ORIGINAL
	end)
	sdk.hook(methods.after_area_act, function(args)
	...
	sdk.hook(methods.cancel_button, function(args) end, function(retval)
		if vars.close_trigger then
	...
	sdk.hook(methods.decide_button, function(args) end, function(retval)
	...
	sdk.hook(methods.quest_activate, function(args)
		if config.current.auto_quest.posting_method ~= 3 then
			local qi = sdk.get_managed_singleton(
				"snow.gui.fsm.questcounter.GuiQuestCounterFsmManager"
			).requestQuestIdentifier
			if
				vars.posting
				and config.current.auto_quest.posting_method == 1
				and dump.quest_data_list[config.current.auto_quest.quest_no]["category"]
					== "Special Random Mystery"
			then
				qi._IsSpecialRandomMystery = true
				qi._ActiveSpecialRandomQuestLv = 300
			end
			config.current.auto_quest.quest_no = qi._QuestNo
				.. (qi._IsSpecialRandomMystery and "S" or "")
		end
	end)

	sdk.hook(methods.get_selected_quest, function(args) end, function(retval)
		if vars.posting then
			if
				config.current.auto_quest.posting_method == 2
					and (vars.quest_type ~= "Random Mystery" and vars.quest_type ~= "Special Random Mystery")
				or config.current.auto_quest.posting_method == 1
			then
				if not quest_data then
					quest_data = methods.get_quest_data:call(
						singletons.questman,
						sdk.create_int32(
	...
	sdk.hook(methods.routine_quit, function(args)
		config.save()
	end)

	sdk.hook(methods.quest_start, function(args)
	...
	sdk.hook(methods.update_yn_window, function(args) end, function(retval)
		if vars.posting then
			return sdk.to_ptr(0)
		else
			return retval
	...
	re.on_frame(function()
		if singletons.spacewatcher then
			...
			if
				vars.game_state == 4
				and vars.prev_game_state ~= 4
				and vars.prev_game_state ~= 3
				and vars.prev_game_state ~= nil
			then
				...
				if dump.ed and no and dump.quest_data_list[no] then
					if not dump.quest_data_list[no]["completed"] then
						dump.quest_data_list[no]["completed"] = methods.is_quest_clear:call(
							singletons.progquestman,
							functions.sanitize_quest_no(no)
						)
					end
				end
				...
				if config.current.auto_quest.auto_post then
