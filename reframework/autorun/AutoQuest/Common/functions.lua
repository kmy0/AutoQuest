...
local quest_board_cmd_id =
	sdk.find_type_definition("snow.gui.GuiManager.OtherCmdId"):get_field("QuestBoard"):get_data(nil)
...
function functions.post_info_message(message)
	methods.post_info_message:call(
		singletons.chatman,
		"<COL YEL>AutoQuest</COL>\n" .. message,
		true and 2412657311
	)
end
...
function functions.set_random_myst_lvl_to_max()
	config.current.auto_quest.anomaly_investigation_max_lv =
		methods.get_mystery_research_level:call(singletons.progman)
end
	...
	if original_type == "table" then
		if copies[original] then
			...
			for original_key, original_value in next, original, nil do
				copy[functions.deep_copy(original_key, copies)] =
					functions.deep_copy(original_value, copies)
			end
			setmetatable(copy, functions.deep_copy(getmetatable(original), copies))
		end
