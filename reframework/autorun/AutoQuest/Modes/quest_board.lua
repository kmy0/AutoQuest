...
local function get_quest_board_menu(target_id, target_field)
	local menu = {}
	menu.list =
		singletons.quest_counter:get_field(quest_board_menu_fields[target_field]["menu_list"])
	...
	vars.cursor =
		singletons.quest_counter:get_field(quest_board_menu_fields[target_field]["cursor"])
...
local function set_random_mystery_fields_special()
	local settings = {
		hunter_num = tonumber(
			dump.hunter_num_array[config.current.auto_quest.anomaly_investigation_hunter_num]
		),
		monster = dump.anomaly_investigations_main_monsters[dump.anomaly_investigations_main_monsters_array[config.current.auto_quest.anomaly_investigation_monster]],
	}
	...
	if config.current.auto_quest.anomaly_investigation_target_type == 1 then
		if config.current.auto_quest.anomaly_investigation_monster == 2 then
			...
			if research_request then
				...
				local monster_min_lvl = methods.get_limit_lvl:call(
					mysterylabo,
					research_request:get_field("_QuestCondition")
				)
			...
			if settings.monster then
				local monster_min_lvl = methods.get_monster_min_lvl_appearance:call(
					singletons.questman,
					settings.monster
				)
				if settings.min_lvl < monster_min_lvl then
		...
		local item_name =
			dump.afflicted_materials.array[config.current.auto_quest.anomaly_investigation_material]
...
function quest_board.switch()
	function functions.post_quest()
		...
		then
			...
			if
				config.current.auto_quest.join_multi_type == 1
				and not methods.is_online:call(singletons.lobbyman)
			then
			...
			elseif
				config.current.auto_quest.posting_method == 3
				and config.current.auto_quest.join_multi_type == 7
				and not dump.quest_data_list[config.current.auto_quest.quest_no]
			then
				functions.error_handler("Invalid Quest ID.")
			elseif
				config.current.auto_quest.posting_method == 3
				and config.current.auto_quest.join_multi_type == 7
				and not dump.quest_data_list[config.current.auto_quest.quest_no]["online"]
			then
...
function quest_board.hook()
	sdk.hook(methods.quest_board_top_start, function(args) end, function(retval)
	...
	sdk.hook(methods.get_quest_data_quest_counter, function(args)
		if
			vars.posting
	...
	sdk.hook(methods.quest_board_on_destroy, function(args)
		if config.current.auto_quest.posting_method == 3 then
			if
				vars.posting
	...
	sdk.hook(methods.recieve_chat_info, function(args)
		if
			config.current.auto_quest.posting_method == 3
			and config.current.auto_quest.join_multi_type == 1
			...
			if vars.posting then
				if config.current.auto_quest.join_multi_type ~= 1 then
					...
					if singletons.quest_counter then
						...
						local menu = {}
						menu.name =
							quest_board_menu_id[quest_board_rank][config.current.auto_quest.join_multi_type]
						...
						menu.index =
							quest_board_menu_id[quest_board_rank][config.current.auto_quest.join_multi_type][menu.type]
						menu.id =
							quest_board_menu_id[quest_board_rank][config.current.auto_quest.join_multi_type]["id"][menu.type]
						menu.current =
						...
						then
							methods.tree_set_node_by_id:call(
								singletons.quest_counter:get_field(
									"<refQuestCounterBehaviorTree>k__BackingField"
								),
								1107141891,
								0,
								nil
						...
						if
							(menu.current == menu.id or not menu.id)
							and indexes.order <= #menu.order
						then
				...
				elseif
					config.current.auto_quest.join_multi_type == 1 and singletons.quest_board
				then
					if methods.is_open_yn:call(singletons.guiman) then
						...
						if actions.select_hub_quest_slot then
							vars.cursor =
								menu_list_type_def:get_field("_Cursor"):get_data(quest.list)
							methods.menu_list_cursor_set_index:call(
								vars.cursor,
								indexes.hub_quest_list
							)
						...
						if vars.selected then
							...
							if quest.hunter_info then
								if
									methods.quest_check:call(quest.list, indexes.hub_quest_list)
									== 0
								then
									quest.member_list =
										singletons.quest_board:get_field("_MemberListCtrl")
									quest.member_list_scroll =
										quest.member_list:get_field("_MemberList")
									quest.member_count = quest.member_list_scroll
										:get_field("_QuestMemberInfoList")
										:get_Count()
