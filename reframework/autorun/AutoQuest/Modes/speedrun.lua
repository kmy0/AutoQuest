        ...
        and not methods.is_quest_posted:call(singletons.questman) then
            ...
            not config.current.auto_quest.auto_randomize then
                if dump.quest_data_list[tonumber(config.current.auto_quest.quest_no)] then
                    ...
                    vars.posting = true
                    singletons.quest_counter = sdk.create_instance('snow.gui.fsm.questcounter.GuiQuestCounterFsmManager')

                    methods.quest_counter_awake:call(singletons.quest_counter)
                    if not config.current.auto_quest.keep_rng then methods.reset_quest_identifier:call(singletons.quest_counter) end
