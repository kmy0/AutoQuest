    ...
    function functions.post_quest()
        if methods.can_open_quest_board:call(singletons.guiman) then
            if not methods.is_quest_posted:call(singletons.questman) then
                quest_counter_obj_loop.name = quest_counter_obj_ids[ singletons.vilman:get_field('<_CurrentAreaNo>k__BackingField') ]
...
function quest_counter.hook()
    ...
    sdk.hook(methods.quest_id_reset,
        function(args)
            if config.current.auto_quest.posting_method == 2
            and vars.posting
            and config.current.auto_quest.keep_rng then
    ...
    sdk.hook(methods.pop_sensor_check_access,
        ...
        function(retval)
            if config.current.auto_quest.posting_method == 2
            and vars.posting
            and actions.select_quest_counter then
    ...
    sdk.hook(methods.interact_button,function(args)end,
        function(retval)
            if config.current.auto_quest.posting_method == 2
            and vars.interact_trigger then
                return sdk.to_ptr(true)
            else
                return retval
            end
