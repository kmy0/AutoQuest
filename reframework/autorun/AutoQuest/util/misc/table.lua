...
---@generic K, V
---@param t table<K, V>
---@param predicate fun(value: V): boolean | V
---@return V?
function this.find(t, predicate)
    for _, v in pairs(t) do
        if (type(predicate) == "function" and predicate(v)) or v == predicate then
            return v
        end
    end
end
...
---@param t table<K, V>
---@return table<V, V>
function this.map_value_to_value(t)
    local ret = {}
    for _, v in pairs(t) do
        ret[v] = v
    end
