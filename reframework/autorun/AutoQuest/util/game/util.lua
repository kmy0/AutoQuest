...
---@generic K, V
---@param dict {[K]: V}
---@param something fun(dict: System.Dictionary<K, V>, key: K, value: V): boolean?
function this.do_something_dict(dict, something)
    local entries = dict._entries --[[@as System.Array<System.DictionaryEntry>]]
    local size = entries:get_Count() - 1
    for i = 0, size do
        local entry = entries:get_Item(i) --[[@as System.DictionaryEntry]]
        if something(dict, entry.key, entry.value) == false then
            break
        end
    end
end
...
function this.get_language()
    return s.get_native("via.gui.GUISystem"):get_MessageLanguage()
end
