...
function native_config_menu.draw()
	...
	then
		...
		button = mod_menu.Button(
			not native_config_menu.randomizer_options_toggle
					and "<COL AUTOQUESTBR>Select Randomizer Chekboxes</COL>"
				or native_config_menu.randomizer_options_toggle
					and "<COL AUTOQUESTBR>Unselect Randomizer Chekboxes</COL>",
			"",
			false,
		...
		if button then
			native_config_menu.randomizer_options_toggle =
				not native_config_menu.randomizer_options_toggle
		...
		mod_menu.Label(
			not native_config_menu.randomizer_options_toggle
					and "<COL GRAY>Select Randomizer Chekboxes</COL>"
				or native_config_menu.randomizer_options_toggle
					and "<COL GRAY>Unselect Randomizer Chekboxes</COL>",
			"",
			"Quest Counter, Speedrun Modes and Quest Board Specific Quest only."
		)
		mod_menu.Label(
	...
	button = mod_menu.Button(
		not bind.new_bind_trigger
				and "<COL AUTOQUESTBR>" .. config_menu.btn_text .. " Quest Bind</COL>"
			or bind.new_bind_trigger
				and "<COL AUTOQUESTGREEN>" .. config_menu.btn_text .. " Quest Bind</COL>",
		not bind.new_bind_trigger and config.current.button_bind.button_name
			or bind.new_bind_trigger
				and "<COL GRAY>Press any key... " .. bind.timer_string .. "</COL>",
		false,
	...
	then
		...
		if button then
			native_config_menu.show_anomaly_investigations_options =
				not native_config_menu.show_anomaly_investigations_options
			mod_menu.Repaint()
		end
		...
		if native_config_menu.show_anomaly_investigations_options then
			...
			changed, config.current.auto_quest.anomaly_investigation_cap_max_lvl =
				mod_menu.CheckBox(
	...
	then
		...
		if button then
			native_config_menu.show_hub_quest_options =
				not native_config_menu.show_hub_quest_options
			mod_menu.Repaint()
		end
	...
	if native_config_menu.show_quest_ranks then
		...
		if not config.current.randomizer.exclude_master_rank then
			...
			if button then
				native_config_menu.show_master_rank_level =
					not native_config_menu.show_master_rank_level
				mod_menu.Repaint()
			end
		else
			mod_menu.Label(
	...
	if native_config_menu.show_quest_categories then
		...
		if not config.current.randomizer.exclude_anomaly then
			...
			if button then
				native_config_menu.show_exclude_anomaly_level =
					not native_config_menu.show_exclude_anomaly_level
				mod_menu.Repaint()
			end
		else
			mod_menu.Label(
		...
		if not config.current.randomizer.exclude_anomaly_investigations then
			...
			if button then
				native_config_menu.show_exclude_anomaly_investigations_level =
					not native_config_menu.show_exclude_anomaly_investigations_level
				mod_menu.Repaint()
			end
		else
			mod_menu.Label(
		...
		if not config.current.randomizer.exclude_arena then
			...
			if button then
				native_config_menu.show_exclude_arena_ranks =
					not native_config_menu.show_exclude_arena_ranks
				mod_menu.Repaint()
			end
		else
			mod_menu.Label(
		...
		if not config.current.randomizer.exclude_rampage then
			...
			if button then
				native_config_menu.show_exclude_rampage_ranks =
					not native_config_menu.show_exclude_rampage_ranks
				mod_menu.Repaint()
			end
		else
			mod_menu.Label(
		...
		if not config.current.randomizer.exclude_event then
			...
			if button then
				native_config_menu.show_exclude_event_ranks =
					not native_config_menu.show_exclude_event_ranks
				mod_menu.Repaint()
			end
		else
			mod_menu.Label(
	...
	then
		...
		button = mod_menu.Button("<COL YEL>Other</COL>", "", false, "Show/Hide Other options.")
		if button then
	...
	if native_config_menu.show_other then
		...
		button = mod_menu.Button(
			...
			false,
			"You have "
				.. functions.table_length(config.current.randomizer.posted_quests) - 1
				.. " quests in the list."
		)
		if button then
...
function native_config_menu.init()
	...
	native_UI = mod_menu.OnMenu("AutoQuest", "Posts quests for you :)", native_config_menu.draw)

	re.on_frame(function()
		if native_config_menu.active and not methods.is_open_startmenu:call(singletons.guiman) then
